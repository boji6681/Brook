...
版本列表请去这里获取：${Green_font_prefix}[ https://github.com/boji6681/brook/releases ]${Font_color_suffix}"
	read -e -p "直接回车即自动获取:" brook_new_ver
	if [[ -z ${brook_new_ver} ]]; then
		brook_new_ver=$(wget -qO- https://api.github.com/repos/boji6681/brook/releases| grep "tag_name"| head -n 1| awk -F ":" '{print $2}'| sed 's/\"//g;s/,//g;s/ //g')
...
Download_brook(){
	...
	cd ${file}
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -N "https://github.com/boji6681/brook/releases/download/${brook_new_ver}/brook"
	else
		wget --no-check-certificate -N "https://github.com/boji6681/brook/releases/download/${brook_new_ver}/brook_linux_386"
...
Service_brook(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/brook-pf_centos -O /etc/init.d/brook-pf; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/brook-pf_debian -O /etc/init.d/brook-pf; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/brook-pf
...
Update_brook(){
 ...
 ${Green_font_prefix}1.${Font_color_suffix}  国内服务器(失效)
 ${Green_font_prefix}2.${Font_color_suffix}  国外服务器(Github)
 
 ${Tip} 因为国内对 Github 限速，这会导致国内服务器下载速度极慢，所以选择 国内服务器 选项就会从我的 失效 下载!" && echo
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/boji6681/brook/master/brook.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/brook-pf" ]]; then
		rm -rf /etc/init.d/brook-pf
		Service_brook
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/boji6681/brook/master/brook.sh" && chmod +x brook.sh
...
if [[ "${action}" == "monitor" ]]; then
	...
	echo && echo -e "┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃ Brook端口转发  一键管理脚本${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃   机场搭建 ❤ 机场托管 ❤ 技术支持 　┃ 
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃  Telegram：https://t.me/bojiiking  ┃
